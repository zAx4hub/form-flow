/** form-flow — engine by zAx4hub (algo flavor: jaccard) */
...
export function rank(text: string, seed = 80): number {
...
export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "form-flow" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 80;
  ...
  return {
    ...
    author: "zAx4hub",
    algo: "jaccard",
    summary: `Processed ${findings.length} items for form-flow; avg=${avg.toFixed(3)}`,
...
export function demo(): Report {
  return run({
    items: [
      { text: "Self-host forms + spam + webhooks" },
...
export function inspect() {
  return {
    name: "form-flow",
    author: "zAx4hub",
    oneLiner: "Self-host forms + spam + webhooks",
    algo: "jaccard",
    version: "0.1.0",
